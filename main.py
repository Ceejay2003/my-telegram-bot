...
async def verify_txid_on_blockchain(txid: str, crypto: str, context: ContextTypes.DEFAULT_TYPE = None) -> bool:
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            # USDT forwarded to underlying chain
...
async def handle_txid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db_session() as session:
        ...
        }.get(context.user_data["selected_plan"], 0)
...
async def admin_panel(update: Update | Message, context: ContextTypes.DEFAULT_TYPE):
    caller = update.effective_user.id if isinstance(update, Update) else update.from_user.id
    if caller != ADMIN_ID:
        await (update.message or update).reply_text(get_msg("en", "admin_not_auth"))
        return ConversationHandler.END
...
async def handle_ad_target(update: Update | Message, context: ContextTypes.DEFAULT_TYPE):
    ...
    if media:
        type_, mid = media
        if type_ == "photo":
            await context.bot.send_photo(chat_id=update.effective_chat.id, photo=mid, caption=text)
        else:
            await context.bot.send_video(chat_id=update.effective_chat.id, video=mid, caption=text)
    else:
...
async def admin_back(update: Update | Message, context: ContextTypes.DEFAULT_TYPE):
